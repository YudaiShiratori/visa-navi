...
このテンプレートをベースにClaude Codeで開発する場合、以下のプラグインのインストールを**強く推奨**します。カスタムコマンドやスキルファイルの一部はこれらのプラグインに依存しています（例: `/pr-review` コマンドは `code-review` プラグインを使用）。

**初回参加時に必ずインストールしてください**:
...
| `code-review` | コードレビュー自動化（`/pr-review` で使用） |
...
- プラグインがインストールされていない場合、一部のカスタムコマンドが正常に動作しません
